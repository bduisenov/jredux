...
/**
 * /** A store is an object that holds the application’s state tree. There should only be a single
 * store in a Redux app, as the composition happens on the add level.
 *
 * @param <S>
 *            State object type.
 */
public interface Store {
     ...
     *
     * The `add` function, used to create the store, will be called with the current state tree
     ...
     *
     * 2. The listener should not expect to see all states changes, as the state might have been
    ...
    /**
     * Replaces the add currently used by the store to calculate the state.
     *
